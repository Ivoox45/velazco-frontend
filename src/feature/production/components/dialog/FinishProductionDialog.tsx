// src/production/components/dialog/FinishProductionDialog.tsx
import {
  Dialog,
  DialogContent,
  DialogHeader,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="w-full max-w-[95vw] sm:max-w-[600px] md:max-w-[750px] max-h-[97vh] overflow-y-auto">
        ...
        {/* Instrucciones */}
        <div className="bg-blue-50 text-blue-900 text-[14px] md:text-[15px] rounded-md px-2 md:px-4 py-2 font-normal mb-5">
        ...
        <div className="space-y-6">
          {products.map((p, idx) => {
            ...
            return (
              <div
                key={idx}
                className="rounded-md border border-gray-200 p-3 md:p-4 bg-white"
              >
                <div className="flex flex-wrap items-center justify-between mb-3 gap-2">
                  <span className="font-semibold text-[16px] md:text-[17px]">
                    {p.producto}
                  </span>
                ...
                <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-3 md:gap-4">
                  {/* Estado */}
                  <div>
                    <div className="font-semibold mb-1 text-[15px] md:text-base">
                      Estado del producto
                    </div>
                    <div className="flex items-center gap-3 md:gap-5">
                      <label className="flex items-center gap-1 cursor-pointer text-[14px] md:text-base">
                        <input
                          type="radio"
                          name={`estado-${idx}`}
                  ...
                  {form.estado === "INCOMPLETO" && (
                    <div className="md:ml-auto flex flex-col w-full md:w-auto md:items-end">
                      ...
                      <input
                        type="number"
                        className="border rounded px-3 py-1 w-full md:w-32 text-[15px] md:text-base"
                        min={0}
                        max={p.cantidad - 1}
                ...
                {form.estado === "INCOMPLETO" && (
                  <div className="w-full mt-4">
                    ...
                    <textarea
                      className="border rounded px-3 py-2 w-full text-[14px] md:text-[15px] resize-none"
        ...
        <div className="mt-8 mb-2">
          <div className="bg-gray-50 border rounded-lg px-3 md:px-6 py-3 md:py-4 grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6 text-[14px] md:text-[15px] items-center">
            <div>
